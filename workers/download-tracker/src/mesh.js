 ...
 * No auto-heal. Not an anonymity network.
 * /v1/mesh/* PROXY to aziel-runtime (AZIEL_RUNTIME binding).
 * Author: Aziel Eliab only.
 */

import { attachWires, GATE_DWELL_S, REHEAL_SPEC, SHELF_SPEC, SURVIVAL_SPEC, TICK_MAX_MS, TICK_MIN_MS, WIRES_SPEC } from "./wires.js";
...
export const MESH_PRODUCT = "azieltether";
...
export const SHELF_SPEC_CITE = "COLD-SHELF-TETHER-1.0";
...
export function meshOpenApiPaths() {
  ...
  for (const route of MESH_PROXY_ROUTES) {
    ...
    for (const method of route.methods) {
      entry[method] = {
        operationId: "azieltether_mesh_" + route.op + (method === "head" ? "_head" : "") + "_proxy",
...
async function originFetch(env, pathAndQuery, init, request) {
  ...
  headers.set("X-Aziel-Runtime-Via", "azieltether-download-tracker");
